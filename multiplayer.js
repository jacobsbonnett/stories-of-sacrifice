...
function showOnlineChoice(){
 ...
 const buttons=cards.map(c=>{const button=document.createElement('button'),description=['assassin','butcher','red-hilt'].includes(choice.kind)?`${c.durability} health`:choice.kind==='sacrifice'?`${Math.ceil(c.cost/2)} Prestige`:c.text;if(['judge-rest','judge-order','blacksmith'].includes(choice.kind)){button.className='online-choice-card';const img=document.createElement('img');img.src=cardArtwork(c);img.alt='';const name=document.createElement('strong');name.textContent=c.name;const text=document.createElement('span');text.textContent=description;button.append(img,name,text);button.setAttribute('aria-label',`${c.name}. ${description}`);}else button.textContent=`${c.name} — ${description}`;button.onclick=()=>sendOnline({type:'choose',id:c.id});return button;});
