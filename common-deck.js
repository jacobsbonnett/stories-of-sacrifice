...
function showCommonChoice(){
 ...
 dialog.querySelector('.choice-list').replaceChildren(...cards.map(c=>{const b=document.createElement('button');b.textContent=`${c.name}${c.durability?` — ${c.durability} health`:''}`;b.onclick=()=>resolveCommonChoice(c.id);return b;}));
