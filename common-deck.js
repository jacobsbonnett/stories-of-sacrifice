...
function showCommonChoice(){
 ...
 dialog.querySelector('.choice-list').replaceChildren(...cards.map(c=>{const b=document.createElement('button');b.className='common-choice-card';const img=document.createElement('img');img.src=cardArtwork(c);img.alt='';const name=document.createElement('strong');name.textContent=c.name;const effect=document.createElement('span');effect.textContent=c.durability?`${c.durability}/${championMaxHealth(c)} health · ${c.text}`:c.text;b.append(img,name,effect);b.setAttribute('aria-label',`${c.name}. ${effect.textContent}`);b.onclick=()=>resolveCommonChoice(c.id);return b;}));
