...
function checkWin(who){const p=who==='player'?state.player:state.ai;const allegiance=state.selected.every(k=>state.legends[k]===who);if(allegiance){finish(who,`${p.name} secured the Allegiance of all four Legends.`);return true}if(p.prestige>=40){const foe=who==='player'?state.ai:state.player;if(state.finale&&state.finale!==who){if(p.prestige>=foe.prestige){finish(who,`${p.name} answered the Prestige challenge and finished with ${p.prestige} Prestige.`);return true}}else state.finale=who}if(state.finale&&state.finale!==who){const challenger=state.finale==='player'?state.player:state.ai;if(p.prestige<challenger.prestige){finish(state.finale,`${challenger.name} reached ${challenger.prestige} Prestige and survived the final response.`);return true}state.finale=null}return false}
function finish(who,text){state.over=true;$('#resultTitle').textContent=who==='player'?'Your sacrifice becomes legend':'The Rival claims the tale';$('#resultText').textContent=text;$('#resultDialog').showModal()}
